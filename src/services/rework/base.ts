 ...
 *
 * Base Rework Service Class
 * 
 * This class provides core functionality for all Rework service modules:
...
/**
 * Custom error class for Rework API errors
 */
export class ReworkServiceError extends Error {
...
/**
 * Rate limit response headers from Rework API
 */
...
/**
 * Base Rework service class that handles common functionality
 */
export class BaseReworkService {
  ...
  /**
   * Creates an instance of BaseReworkService.
   * @param apiKey - Rework API key for authentication
   * @param password - Rework team ID for targeting the correct workspace
   * @param baseUrl - Optional custom base URL for the Rework API
   */
  constructor(apiKey: string, password: string, baseUrl: string = 'https://apis.rework.com') {
    this.apiKey = apiKey;
    ...
    this.logger = new Logger(`Rework:${className}`);
    ...
    this.logger.debug(`Initialized ${className}`, { password, baseUrl });
  ...
  private handleAxiosError(error: any): never {
    ...
    if (error.code === 'ECONNABORTED' || error.message?.includes('timeout')) {
      ...
      logMessage = `Rework server error: ${errorMsg}`;
      errorMessage = 'Rework server error. Please try again later.';
    } else {
    ...
    throw new ReworkServiceError(errorMessage, code, error);
  }

  /**
   * Handle rate limit headers from Rework API
   * @private
   * @param headers Response headers from Rework
   */
  ...
  private async processQueue(): Promise<void> {
    ...
    try {
      ...
      if (error instanceof ReworkServiceError && error.code === ErrorCode.RATE_LIMIT) {
      ...
      return new Promise<T>((resolve, reject) => {
        this.requestQueue.push(async () => {
          try {
            ...
            if (error instanceof ReworkServiceError && error.code === ErrorCode.RATE_LIMIT) {
              const enhancedError = new ReworkServiceError(
