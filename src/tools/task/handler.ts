 ...
 *
 * Rework MCP Task Operation Handlers
 * 
...
import { TaskPriority, UpdateTaskData, TaskFilters, toTaskPriority, CreateTaskData } from '../../services/rework/types.js';
 ...
 */
function estimateTaskResponseTokens(task: ReworkTask): number {
...
function wouldExceedTokenLimit(response: any): boolean {
  ...
  const totalTokens = response.tasks.reduce((sum: number, task: ReworkTask) =>
